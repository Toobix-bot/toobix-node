...
const APPROVED_DIR = path.join(process.cwd(), '_TOOBIX_KNOWLEDGE_SPACE', 'bridge', 'approved');

/**
 * Liest einen pending Snapshot, signiert ihn (v0.1 simpel) und verschiebt ihn nach "approved".
 */
export async function approveSnapshot(req: ApproveSnapshotRequest): Promise<string> {
    ...
    // Speichern im approved Ordner
    const approvedPath = path.join(APPROVED_DIR, req.filename);
    fs.writeFileSync(approvedPath, JSON.stringify(snapshot, null, 2), 'utf-8');
